...
async def test_runner_pending_input_replays_after_sqlalchemy_post_commit_ack_loss(
    ...
    The real SQLAlchemySession commits the staged input durably. A wrapper around the
    Session.add_items API then raises after that commit and before the caller receives
    the acknowledgement, modeling the lost-acknowledgement boundary.
    """
    ...
    async def add_items_with_lost_ack(items: list[TResponseInputItem]) -> None:
        ...
        if (
            simulate_ack_loss
            and any(
                isinstance(item, dict) and item.get("content") == "Late input"
                for item in items
            )
        ):
            simulate_ack_loss = False
